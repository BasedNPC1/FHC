...
// State variables
bool isRunning = false;    // Is the solenoid currently running7
...
const int vib_down_pos = 0; //how far down to go
const int pow_up_pos = 180;
const int pow_down_pos = 90; //how far down to go
...
void setup() {
  ...
  Serial.println("  'f' - Forward burst (1500ms)");
  Serial.println("  'b' - Backward burst (1500ms)");
  Serial.println("  'd' - Direction toggle test (1 toggle 1500ms)");
  ...
  if (Serial.available() > 0) {
    ...
    switch(command) {
      ...
      default:
        ...
        Serial.println("  'f' - Forward burst (1500ms)");
        Serial.println("  'b' - Backward burst (1500ms)");
        Serial.println("  'd' - Direction toggle test (1 toggles 1500ms)");
        Serial.println("  'p' - Fart pattern (quick ramp up, slow fade out)");
...
void forwardBurst() {
  ...
  delay(1500);                     // Run for 100 milliseconds
  //stopSolenoid();
